...
(function () {
  ...
  const StorageService = {
    ...
    saveUserStatus(status) {
      try {
        const trimmed = (status || "").slice(
          0,
          GAME_SETTINGS.STATUS_MAX_LENGTH
        );
  ...
  const BadgeService = {
    ...
    checkAndAwardBadges(stats, additionalContext = {}) {
      ...
      if (stats.mediumWins >= 20 && !this.hasBadge("medium_master")) {
        if (this.awardBadge("medium_master"))
          newlyAwarded.push("medium_master");
      }
      ...
      if (
        ...
        if (this.awardBadge("completionist"))
          newlyAwarded.push("completionist");
      }
