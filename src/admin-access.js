...
(function() {
  ...
  const AdminManager = {
    ...
    async login() {
      console.log('%c🔐 Sudoku Labs Admin Console', 'font-size: 20px; font-weight: bold; color: #ff0000;');
      console.log('%cThis is a secure administrative console.', 'font-size: 14px; color: #ff6600;');
      console.log('%cUnauthorized access is prohibited.', 'font-size: 14px; color: #ff6600;');
      console.log('');

      // Check if already logged in
      if (this.sessionToken && Date.now() < this.sessionExpiry) {
        const remaining = Math.floor((this.sessionExpiry - Date.now()) / 60000);
        console.log(`✓ Already authenticated. Session expires in ${remaining} minutes.`);
      ...
      try {
        ...
        if (data.success && data.token) {
          ...
          this.sessionExpiry = Date.now() + (window.ADMIN_CONFIG?.SESSION_TIMEOUT || 30 * 60 * 1000);
          
          console.log('%c✓ Authentication successful!', 'color: #00ff00; font-weight: bold;');
          console.log(`Session expires at: ${new Date(this.sessionExpiry).toLocaleTimeString()}`);
          console.log('');
          console.log('%cRun sudokuAdmin.open() to access the admin console.', 'font-size: 14px; color: #00ffff;');
        ...
        console.log('');
        console.log('%cNote: Backend admin endpoints may not be deployed yet.', 'color: #ffff00;');
        console.log('Verify GAS_URL is configured and admin functions are deployed.');
      }
    },
    ...
    open() {
      ...
      if (!window.React || !window.AdminConsole) {
        console.error('❌ Admin Console component not loaded. Ensure admin.jsx is included.');
        return;
      }
      ...
      root.render(
        React.createElement(window.AdminConsole, {
    ...
    help() {
      console.log('%c📖 Sudoku Labs Admin Console - Help', 'font-size: 16px; font-weight: bold;');
      console.log('');
  ...
  console.log('%cAdmin console available. Type sudokuAdmin.help() for commands.', 'color: #666;');
