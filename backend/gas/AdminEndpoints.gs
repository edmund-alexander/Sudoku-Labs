...
function updateUserStats(params) {
  ...
  try {
    ...
    const data = usersSheet.getRange(2, 1, usersSheet.getLastRow() - 1, 15).getValues();
    ...
    for (let i = 0; i < data.length; i++) {
      if (data[i][0] === username) {
        ...
        if (params.totalGames) usersSheet.getRange(row, 9).setValue(parseInt(params.totalGames));
        if (params.totalWins) usersSheet.getRange(row, 10).setValue(parseInt(params.totalWins));
        if (params.easyWins) usersSheet.getRange(row, 11).setValue(parseInt(params.easyWins));
        if (params.mediumWins) usersSheet.getRange(row, 12).setValue(parseInt(params.mediumWins));
        if (params.hardWins) usersSheet.getRange(row, 13).setValue(parseInt(params.hardWins));
        if (params.perfectWins) usersSheet.getRange(row, 14).setValue(parseInt(params.perfectWins));
        if (params.fastWins) usersSheet.getRange(row, 15).setValue(parseInt(params.fastWins));
