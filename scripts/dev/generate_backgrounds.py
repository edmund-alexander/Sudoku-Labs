...
def build_prompt(visual: str, audio: str, style_override: str = None) -> str:
    ...
    style_instruction = (
        f"Art Style: {style_override}. " if style_override
        else "Design goals: subtle, abstract, non-figurative visuals using gradients, soft shapes, and light textures. "
    )
...
def generate_local_background(visual: str, audio: str, out_img: Path, width=1920, height=1080):
    ...
    for _ in range(blob_count):
        ...
        blob_c = tuple(max(0, min(255, int(ch * (1 + hue_shift))))
                       for ch in blob_c)
        blob = Image.new("RGBA", (width, height), (0, 0, 0, 0))
        bdraw = ImageDraw.Draw(blob)
        bdraw.ellipse([(bx - br, by - br), (bx + br, by + br)],
                      fill=blob_c + (opacity,))
    ...
    def draw_ocean(pixelated=False):
        ...
        for i in range(8):
            ...
            for x in range(0, width, step):
                t = x / width
                yy = y + amp * (rng.uniform(0.8, 1.2)) * \
                    (math.sin(freq * t * 3.14 + phase))
                if pixelated:
                    yy = int(yy / 16) * 16
                pts.append((x, yy))
        ...
        if not minimal_style:
            for _ in range(creature_count):
                ...
                if pixelated:
                    ...
                    x = int(x / 16) * 16
                    y = int(y / 16) * 16
                    mdraw.rectangle([(x, y), (x+size*2, y+size)],
                                    fill=(255, 255, 255, alpha))
                    mdraw.polygon([(x+size*2, y), (x+size*3, y+size//2),
                                  (x+size*2, y+size)], fill=(255, 255, 255, alpha))
                else:
                    # Bubbles
                    mdraw.ellipse(
                        [(x-size//2, y-size//2), (x+size//2, y+size//2)], fill=(255, 255, 255, alpha))
    ...
    def draw_midnight(pixelated=False, playful=False):
        ...
        if pixelated:
            ...
            for i in range(5):
                ...
                alpha = 80 - i * 15
                mdraw.ellipse(
                    [(moon_x-r, moon_y-r), (moon_x+r, moon_y+r)], fill=(255, 255, 255, alpha))
        ...
        for _ in range(star_count):
            ...
            if pixelated:
                ...
                mdraw.rectangle([(x, y), (x+size, y+size)],
                                fill=(255, 255, 255, alpha))
            elif playful:
                r = rng.choice([2, 3, 4, 5])
                alpha = rng.randint(50, 130)
                color = (255, rng.randint(200, 255),
                         rng.randint(180, 255), alpha)
        ...
        if not minimal_style:
            ...
            for _ in range(constellation_count):
                ...
                for i in range(len(points)-1):
                    mdraw.line([points[i], points[i+1]], fill=(255,
                               255, 255, 25), width=2 if pixelated else 1)
    ...
    def draw_sakura(pixelated=False, flowing=False):
        ...
        for _ in range(petal_count):
            ...
            if pixelated:
                ...
                center = size // 2
                mdraw.rectangle(
                    [(cx-center, cy-center), (cx+center, cy+center)], fill=(255, 220, 240, alpha))
                mdraw.rectangle(
                    [(cx-size, cy-center//2), (cx+size, cy+center//2)], fill=(255, 200, 230, alpha))
                mdraw.rectangle(
                    [(cx-center//2, cy-size), (cx+center//2, cy+size)], fill=(255, 200, 230, alpha))
            else:
                ...
                for i in range(5):
                    ...
                    pdraw.ellipse(
                        [(px-rx*0.5, py-ry*0.5), (px+rx*0.5, py+ry*0.5)], fill=(255, 220, 240, alpha))
                if not flowing:
                    petal = petal.rotate(
                        rot*57.3, resample=Image.BICUBIC, center=(cx, cy))
    ...
    def draw_arctic(geometric=True, pixelated=False):
        ...
        for _ in range(ice_count):
            ...
            if pixelated:
                ...
                for i in range(steps):
                    ...
                    x1 = int(x1 / 8) * 8
                    x2 = int(x2 / 8) * 8
                    mdraw.rectangle([(x1, y), (x2, y+16)],
                                    fill=(255, 255, 255, 50+i*3))
            else:
                # Smooth icicle
                poly = [
                    (x, top_y),
        ...
        for _ in range(crystal_count):
            ...
            size = rng.uniform(
                20, 50) if not pixelated else rng.choice([16, 24, 32])
            ...
            if pixelated:
                ...
                s = int(size / 8) * 8
                # Cross pattern
                mdraw.rectangle([(cx-s, cy-s//4), (cx+s, cy+s//4)],
                                fill=(255, 255, 255, alpha))
                mdraw.rectangle([(cx-s//4, cy-s), (cx+s//4, cy+s)],
                                fill=(255, 255, 255, alpha))
                # Diagonal arms
                for i in range(-s//2, s//2, 8):
                    mdraw.rectangle(
                        [(cx+i-4, cy+i-4), (cx+i+4, cy+i+4)], fill=(255, 255, 255, alpha-20))
                    mdraw.rectangle(
                        [(cx+i-4, cy-i-4), (cx+i+4, cy-i+4)], fill=(255, 255, 255, alpha-20))
            elif geometric:
                # 6-pointed snowflake
                for i in range(6):
                    ...
                    for t in [0.4, 0.7]:
                        ...
                        for ba in [angle - 0.5, angle + 0.5]:
                            ...
                            mdraw.line([(bx, by), (bx2, by2)], fill=(
                                255, 255, 255, alpha-20), width=1)
            else:
                # Simple crystal
                poly = []
                for i in range(6):
                    ...
                    poly.append((cx + radius*math.cos(angle),
        ...
        if audio in ("nature", "space"):
            ...
            for i in range(animal_count):
                ...
                if pixelated:
                    ...
                    head_h = scale * 0.5
                    mdraw.rectangle(
                        [(ax, ay - body_h), (ax + body_w, ay)], fill=(255, 255, 255, 70))
                    mdraw.rectangle(
                        [(ax + body_w, ay - head_h), (ax + body_w + head_w, ay)], fill=(255, 255, 255, 80))
                    leg_w = scale * 0.25
                    for lx in [ax + scale*0.2, ax + scale*0.8, ax + scale*1.4, ax + scale*1.9]:
                        mdraw.rectangle(
                            [(lx, ay), (lx + leg_w, ay + scale*0.35)], fill=(255, 255, 255, 80))
                else:
                    ...
                    if is_penguin:
                        ...
                        # Body
                        mdraw.ellipse(
                            [(ax - body_w/2, ay - body_h), (ax + body_w/2, ay)], fill=(255, 255, 255, 60))
                        # Wings
                        mdraw.polygon([(ax - body_w*0.6, ay - body_h*0.4), (ax - body_w*1.1, ay -
                                      body_h*0.2), (ax - body_w*0.5, ay - body_h*0.1)], fill=(255, 255, 255, 50))
                        mdraw.polygon([(ax + body_w*0.6, ay - body_h*0.4), (ax + body_w*1.1, ay -
                                      body_h*0.2), (ax + body_w*0.5, ay - body_h*0.1)], fill=(255, 255, 255, 50))
                        # Head
                        mdraw.ellipse([(ax - head_r, ay - body_h - head_r*0.8), (ax +
                                      head_r, ay - body_h + head_r*0.8)], fill=(255, 255, 255, 70))
                        # Feet
                        mdraw.rectangle(
                            [(ax - body_w*0.25, ay), (ax + body_w*0.25, ay + scale*0.15)], fill=(255, 255, 255, 70))
                    else:
                        ...
                        head_r = scale * 0.35
                        mdraw.ellipse(
                            [(ax, ay - body_h), (ax + body_w, ay)], fill=(255, 255, 255, 55))
                        mdraw.ellipse([(ax + body_w - head_r*1.4, ay - body_h - head_r), (ax +
                                      body_w + head_r*0.6, ay - body_h + head_r)], fill=(255, 255, 255, 70))
                        # Legs
                        leg_w = scale * 0.25
                        for lx in [ax + scale*0.2, ax + scale*0.9, ax + scale*1.6, ax + scale*2.0]:
                            mdraw.rectangle(
                                [(lx, ay), (lx + leg_w, ay + scale*0.25)], fill=(255, 255, 255, 70))
    ...
    def draw_forest(pixelated=False):
        ...
        for _ in range(leaf_count):
            ...
            if pixelated:
                ...
                mdraw.rectangle(
                    [(cx-size, cy-size//2), (cx+size, cy+size//2)], fill=(200, 255, 200, alpha))
            else:
                rx = rng.uniform(8, 24)
                ry = rx * rng.uniform(0.5, 0.8)
                rot = rng.uniform(0, 3.14)
    ...
    def draw_sunset(pixelated=False):
        ...
        if pixelated:
            ...
            for i in range(6):
                ...
                alpha = 90 - i * 12
                mdraw.ellipse(
                    [(sun_x-r, sun_y-r), (sun_x+r, sun_y+r)], fill=(255, 220, 180, alpha))
        ...
        for i in range(ray_count):
            ...
            if pixelated:
                ...
                ex = int(ex / 16) * 16
                ey = int(ey / 16) * 16
                mdraw.line([(sx, sy), (ex, ey)], fill=(
                    255, 200, 150, 50), width=6)
            else:
                # Draw gradient rays
                segments = 8
        ...
        if audio == "space":
            ...
            for _ in range(ship_count):
                ...
                if pixelated:
                    ...
                    nose_w = scale * 0.5
                    mdraw.rectangle(
                        [(sx - body_w/2, sy - body_h/2), (sx + body_w/2, sy + body_h/2)], fill=(255, 255, 255, 70))
                    mdraw.polygon([(sx + body_w/2, sy - body_h/2), (sx + body_w/2 + nose_w, sy),
                                  (sx + body_w/2, sy + body_h/2)], fill=(255, 255, 255, 80))
                    # Thruster trail
                    mdraw.rectangle([(sx - body_w/2 - scale*0.3, sy - body_h*0.2),
                                    (sx - body_w/2, sy + body_h*0.2)], fill=(255, 200, 150, 60))
                else:
                    body_w = scale * 1.2
                    body_h = scale * 0.7
                    ...
                    mdraw.polygon([tail, (sx - body_w*0.3, sy - body_h*0.6), nose,
                                  (sx - body_w*0.3, sy + body_h*0.6)], fill=(255, 255, 255, 70))
                    # Windows
                    win_r = scale * 0.1
                    for j in range(3):
                        ...
                        wy = sy
                        mdraw.ellipse(
                            [(wx - win_r, wy - win_r), (wx + win_r, wy + win_r)], fill=(255, 200, 180, 80))
                    # Exhaust trail
                    trail_len = scale * 0.9
                    mdraw.polygon([
                        (tail[0] - trail_len, sy),
        ...
        if not minimal_style and not pixelated:
            for _ in range(3):
                ...
                for i in range(4):
                    ...
                    mdraw.ellipse([(ox-r, cy-r//2), (ox+r, cy+r//2)],
                                  fill=(255, 255, 255, 20))
            ...
            if pixelated:
                ...
                for _ in range(lava_streams):
                    ...
                    for j in range(0, stream_height, 16):
                        ...
                        mdraw.rectangle([(lx, ly), (lx+16, ly+16)],
                                        fill=(255, 180, 100, rng.randint(80, 140)))
            else:
                # Smooth volcano
                poly = [
                ...
                for _ in range(lava_count):
                    ...
                    for i in range(8):
                        ...
                        w = rng.randint(3, 8)
                        mdraw.ellipse(
                            [(x_drift-w, y-w), (x_drift+w, y+w)], fill=(255, 180, 100, alpha))
    ...
    def add_grid(count=40):
        ...
        for x in range(0, width, step):
            if rng.random() < 0.35:
                mdraw.line([(x, 0), (x, height)], fill=(
                    255, 255, 255, alpha), width=1)
        for y in range(0, height, step):
            if rng.random() < 0.35:
                mdraw.line([(0, y), (width, y)], fill=(
                    255, 255, 255, alpha), width=1)
    ...
    for i in range(10):
        ...
        alpha = int(180 * (i / 9))
        vdraw.rectangle([inset, inset, width - inset,
                        height - inset], outline=alpha, width=3)
...
def generate_api_background(prompt: str, out_img: Path, api_key: str, model: str):
    """
    Generate background using Hugging Face Inference API (DeepFloyd IF or compatible).
    """
    # Default to DeepFloyd IF if not specified, but allow override
    # Note: DeepFloyd IF is a multi-stage model. Using the standard Inference API
    # on the stage 1 model will result in 64x64 images.
    # For a better experience with a single call, we might default to a model that does
    # the full pipeline or use a provider that handles it.
    # However, per user request, we target "IF by DeepFloyd Lab".
    ...
    raise Exception("API generation failed")


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--root", default="public/assets/themes",
                        help="Root assets directory")
    parser.add_argument("--format", default="png",
                        choices=["png", "jpg"], help="Output image format")
    parser.add_argument("--model", default="DeepFloyd/IF-I-XL-v1.0",
                        help="Hugging Face model ID (default: DeepFloyd/IF-I-XL-v1.0)")
    parser.add_argument("--dry-run", action="store_true",
                        help="Skip writing images (placeholder only)")
    parser.add_argument("--limit", type=int, default=None,
                        help="Limit number of images to generate (for testing)")
    parser.add_argument("--clean", action="store_true",
                        help="Remove existing background images before generation")
    parser.add_argument("--skip-existing", action="store_true",
                        help="Skip generation if background image already exists")
    parser.add_argument("--style", type=str, default=None,
                        help="Override art style description (e.g. 'flat geometric', 'pixel art')")
    ...
    parser.add_argument("--use-api", action="store_true",
                        help="Use Hugging Face Inference API instead of local generation")
    ...
    if not PIL_AVAILABLE:
        print("Pillow not installed; cannot generate local backgrounds.",
              file=sys.stderr)
        sys.exit(1)

    if args.use_api and not args.api_key:
        print("Error: --use-api requires --api-key or HF_API_KEY environment variable", file=sys.stderr)
        sys.exit(1)
    ...
    for visual in VISUALS:
        for audio in AUDIOS:
            ...
            if args.skip_existing and out_img.exists():
                print(f"-> {visual}/{audio}: Skipping (exists)")
                continue

            prompt = build_prompt(visual, audio, args.style)
            ...
            try:
                if args.use_api:
                    generate_api_background(
                        prompt, out_img, args.api_key, args.model)
                    print(f"  ✓ Generated {out_img.name} (API: {args.model})")
                else:
    ...
    print(f"Done. Generated {generated} assets.")
